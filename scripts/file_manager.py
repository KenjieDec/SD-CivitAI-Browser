...
print_ly = lambda x: print("\033[93mCivBrowser: " + x + "\033[0m")
print_lc = lambda x: print("\033[96mCivBrowser: " + x + "\033[0m")
...
def chkfldr(content_type, use_new_folder, model_name = ""):
    ...
    folders = {
        ...
        "LoCon": cmd_opts.lyco_dir if "lyco_dir" in cmd_opts else cmd_opts.lyco_dir_backcompat if "lyco_dir_backcompat" in cmd_opts else os.path.join(models_path, "LyCORIS"),
        "VAE": cmd_opts.vae_dir if cmd_opts.vae_dir else os.path.join(models_path, "VAE"),
        "Controlnet": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "ControlNet") if cmd_opts.ckpt_dir else os.path.join(models_path, "ControlNet"),
        "Poses": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "Poses") if cmd_opts.ckpt_dir else os.path.join(models_path, "Poses"),
        "Upscaler": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "Upscaler") if cmd_opts.ckpt_dir else os.path.join(models_path, "Upscaler"),
        "MotionModule": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "MotionModule") if cmd_opts.ckpt_dir else os.path.join(models_path, "MotionModule"),
        "Wildcards": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "Wildcards") if cmd_opts.ckpt_dir else os.path.join(models_path, "Wildcards"),
        "Workflows": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "Workflows") if cmd_opts.ckpt_dir else os.path.join(models_path, "Workflows"),
        "Other": os.path.join(os.path.join(cmd_opts.ckpt_dir, os.pardir), "Other") if cmd_opts.ckpt_dir else os.path.join(models_path, "Other")
    }
...
def removeFile(file):
    file_loc = file.replace('/','\\')
    if send2trash_installed:
        send2trash(file_loc)
        print_lc('Move file to trash')
    else:
        os.remove(file_loc)
        print_lc('File is deleted with os since send2trash module is missing.')
    return
